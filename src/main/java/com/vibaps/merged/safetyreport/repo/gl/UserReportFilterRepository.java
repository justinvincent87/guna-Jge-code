...
public interface UserReportFilterRepository extends JpaRepository<UserReportFilterEntity, Integer> {
	...
	@Query(value = "update gl_rulelist a,gl_selectedvalues b,gen_user c,gl_minmiles d set b.status=0,d.minmiles=:minMiles  "
	        + "where  c.companyid=:companyId and b.gen_user_id=c.id and a.id=b.gen_rulelist_id and b.status=1 and d.gen_user_id=c.id", nativeQuery = true)
	...
	@Query(value = "update gl_rulelist a,gl_selectedvalues b,gen_user c set b.status=1, b.weight=:weight "
	        + "where  c.companyid=:companyId and b.gen_user_id=c.id and a.id=b.gen_rulelist_id and a.rulevalue=:exception", nativeQuery = true)
	void updateRule(@Param("companyId") String companyId, @Param("weight") Integer weight,
	...
	void insertUserRuleList(@Param("userid") BigInteger userid, @Param("rule") Integer rule,
	        @Param("status") Integer status, @Param("weight") Integer weight);
	...
	@Query(value = "select a.rulevalue,a.weight,a.rulename from gl_rulelist a,gl_selectedvalues b,gen_user c where c.companyid=:userid and c.db=:db and b.gen_user_id=c.id and a.db=:db and a.id=b.gen_rulelist_id and b.status= 1 and a.rulecompany='L'", nativeQuery = true)
